...
namespace TakeyourStand.Tests
{
    public abstract class TakeyourStandTestBase : AbpIntegratedTestBase<TakeyourStandTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected TakeyourStandTestBase()
        {
        ...
        protected void UsingDbContext(Action<TakeyourStandDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TakeyourStandDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TakeyourStandDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TakeyourStandDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TakeyourStandDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TakeyourStandDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TakeyourStandDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TakeyourStandDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TakeyourStandDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TakeyourStandDbContext, Task<T>> func)
        {
            T result;
